...
class Iron(Building):
	def __init__(self,level=0,*args,**kwargs):
		...
		if self.level > 0:
			self.village.production["iron"] = self.requirements[self.requirements['level'] == self.level].iloc[0].to_dict()["new_production"]
	...
	def upgrade(self):
		...
		self.village.production["iron"] = current_level_requirements["new_production"]
		return result, reward
